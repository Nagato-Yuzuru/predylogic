...
求值时传入 `trace=True`，返回一棵结果树，记录每个原子谓词的判定结果。再关掉短路（`short_circuit=False`），所有分支都会被跑完，一次告诉你哪些条件不满足，而不是碰到第一个不成立就停。适用于审计、排查，或者一次把所有未通过的条件列给用户。详见 [Trace 追踪](guides/trace.md)。

Trace 是特化编译的慢路径，不开启几乎没有额外开销。不开 trace 时，谓词树编译成 Python 字节码，跑原生 opcode，接近手写的 `and` / `or`。原理见[设计](design/index.md)。
